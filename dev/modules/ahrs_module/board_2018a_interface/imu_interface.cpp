...
void IMUInterface::init_mpu6500() {
    // Start SPI driver
    spiStart(&MPU6500_SPI_DRIVER, &SPI_cfg);
    ...
    switch (CONFIG._gyro_scale) {
        case MPU6500_GYRO_SCALE_250:
            gyro_sensitivity = (1.0f / 131.0f);
            break;
        case MPU6500_GYRO_SCALE_500:
            gyro_sensitivity = (1.0f / 65.5f);
            break;
        case MPU6500_GYRO_SCALE_1000:
            gyro_sensitivity = (1.0f / 32.8f);
            break;
        case MPU6500_GYRO_SCALE_2000:
            gyro_sensitivity = (1.0f / 16.4f);
            break;
        default:
    ...
    switch (CONFIG._accel_scale) {
        case MPU6500_ACCEL_SCALE_2G:
            accel_sensitivity = (1 / 16384.0f) * GRAV_CONSTANT;
            break;
        case MPU6500_ACCEL_SCALE_4G:
            accel_sensitivity = (1 / 8192.0f) * GRAV_CONSTANT;
            break;
        case MPU6500_ACCEL_SCALE_8G:
            accel_sensitivity = (1 / 4096.0f) * GRAV_CONSTANT;
            break;
        case MPU6500_ACCEL_SCALE_16G:
            accel_sensitivity = (1 / 2048.0f) * GRAV_CONSTANT;
            break;
        default:
            accel_sensitivity = 0.0f;
            break;
    }
    /// Calibrate gyro zero bias
    int i = 0;
    while(i < GYRO_CALIBRATION_SAMPLE_NUMBER) {
        // Fetch data from SPI
